...
class SparseLogisticRegressionModel(Model):
    def __init__(self, **kwargs):
        super().__init__(CSparseLogisticRegressionModel.__new__(CSparseLogisticRegressionModel, **kwargs))

cdef class CSparseLogisticRegressionModel(CModel):
    """
    ...
    cdef tuple _adam(self, int epoch, int step, object minibatch):
        ...
        cdef Py_ssize_t minibatch_size = len(minibatch[0])
        ...
        cdef list minibatch_samples = minibatch[1]
        cdef cnp.float32_t[::1] minibatch_labels = minibatch[0].astype(np.float32)

        cdef DTYPE_t bias_gradient = 0.0
        cdef DTYPE_t loss = 0.0

        cdef DTYPE_t[:, ::1] sample
        cdef Py_ssize_t idx
        cdef Py_ssize_t n_samples


        for m in range(minibatch_size):
            sample = minibatch_samples[m]
            n_samples = sample.shape[0]
            pred = self.prediction(sample)
            loss += loss_function(pred, minibatch_labels[m])

            for f in range(n_samples):
                idx = int(sample[f, 0])
                gradient[idx] += (pred - minibatch_labels[m]) * sample[f, 1]

            if self.biased:
                bias_gradient += (pred - minibatch_labels[m])
    ...
    @cython.wraparound(False)
    @cython.initializedcheck(False)
    cdef DTYPE_t prediction(self, DTYPE_t[:, ::1] samples):
        cdef Py_ssize_t i
        cdef Py_ssize_t n_samples = samples.shape[0]
        ...
        for i in range(n_samples):
            idx = int(samples[i, 0])
            feature_pred = self._weights[idx] * samples[i, 1]
            y = feature_pred - c
            t = pred + y
            c = (t - pred) - y
            pred = t
